...
import { Camera, AlertTriangle, Cloud, FileText, Check, TrendingUp, TrendingDown, Activity } from 'lucide-react';

// A custom counter hook for smooth number animation
const useCounter = (end, duration = 1) => {
...
const PremiumSummaryCard = ({ 
  ...
  const animatedValue = useCounter(value, 1.5);
  ...
  return (
    ...
    >
      {/* Background Glow */}
      <div className={`absolute top-0 right-0 w-32 h-32 bg-[${themeColor}] opacity-[0.03] rounded-full blur-[30px] group-hover:opacity-[0.08] transition-opacity duration-500`} />
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        ...
        <div className="relative">
          <div className={`absolute inset-0 rounded-[18px] bg-[${themeColor}] opacity-20 blur-md group-hover:opacity-40 transition-opacity duration-500`} />
          <div className="relative w-12 h-12 flex items-center justify-center rounded-[18px] bg-white/60 backdrop-blur-md border border-white/80 shadow-[0_4px_12px_rgba(0,0,0,0.05)] overflow-hidden">
            <div className={`absolute inset-0 bg-gradient-to-br from-[${themeColor}]/10 to-transparent`} />
            <motion.div
              variants={{
            ...
            >
              <Icon className={`w-5 h-5 text-[${themeColor}] relative z-10`} style={{ color: themeColor }} />
            </motion.div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/80 backdrop-blur-sm border border-slate-100 shadow-sm">
          {statusText === 'Live Counter' ? (
            <Activity className={`w-3 h-3 text-[${themeColor}] animate-pulse`} style={{ color: themeColor }} />
          ) : (
            <Check className={`w-3 h-3 text-[${themeColor}]`} style={{ color: themeColor }} />
          )}
...
const EvidenceSummaryCards = ({ data }) => {
  const total = data.length;
  const critical = data.filter(e => e.aqi > 200 || e.confidence > 95).length;
  const uploaded = data.filter(e => e.cloudinaryPublicId).length;
  const reported = data.filter(e => e.reportStatus === 'Generated' || e.reportStatus === 'Sent').length;

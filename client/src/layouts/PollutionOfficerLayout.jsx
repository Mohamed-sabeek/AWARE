...
const pollutionNavItems = [
  { path: '/pollution/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { path: '/pollution/incidents', icon: CloudFog, label: 'Assigned Incidents' },
];

const PollutionOfficerLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-[#F8FBFF] overflow-hidden font-sans selection:bg-[#3B82F6]/20 relative">
      ...
      {/* POLLUTION OFFICER SIDEBAR - Matching Admin/Authority Design System */}
      <motion.aside
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden py-4 px-4 space-y-1 custom-scrollbar">
          {pollutionNavItems.map((item) => {
            const isActive = location.pathname === item.path || 
              (item.path === '/pollution/dashboard' && location.pathname === '/pollution') ||
              (item.path === '/pollution/incidents' && location.pathname.startsWith('/pollution/incidents'));
        ...
        <div className="p-4 border-t border-[#DCEEFF]">
          <button
            ...
            className="w-full flex items-center gap-4 px-4 py-3 rounded-2xl text-[#64748B] hover:text-red-500 hover:bg-red-50/50 transition-colors cursor-pointer group"
          >
...
export default PollutionOfficerLayout;

...
const fireNavItems = [
  { path: '/fire/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { path: '/fire/incidents', icon: Flame, label: 'Assigned Incidents' },
];

const FireOfficerLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-[#F8FBFF] overflow-hidden font-sans selection:bg-[#3B82F6]/20 relative">
      ...
      {/* FIRE OFFICER SIDEBAR - Matching Admin/Authority Design System */}
      <motion.aside
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden py-4 px-4 space-y-1 custom-scrollbar">
          {fireNavItems.map((item) => {
            const isActive = location.pathname === item.path || 
              (item.path === '/fire/dashboard' && location.pathname === '/fire') ||
              (item.path === '/fire/incidents' && location.pathname.startsWith('/fire/incidents'));
...
export default FireOfficerLayout;

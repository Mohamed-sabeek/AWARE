...
const WorkflowNode = memo(({ step, index, total, isInView = true }) => {
  const getMicroAnimation = (id) => {
     ...
     switch(id) {
       ...
       case 4: return <motion.div animate={{ top: ['10%', '90%', '10%'] }} transition={{ duration: 2, repeat: Infinity }} className="absolute left-1/2 -translate-x-1/2 w-10 h-[2px] bg-accent shadow-[0_0_8px_#6FC8FF] pointer-events-none" />;
       case 5: return <motion.div animate={{ opacity: [0, 1, 0] }} transition={{ duration: 3, repeat: Infinity, times: [0, 0.1, 1] }} className="absolute inset-0 bg-white/80 rounded-full pointer-events-none" />;
       case 6: return <motion.div animate={{ y: [4, -4, 4] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 flex items-center justify-center pointer-events-none"><Cloud className="w-10 h-10 text-primary opacity-20 blur-[2px]" /></motion.div>;
       case 7: return <motion.div animate={{ scaleY: [1, 1.3, 1] }} transition={{ duration: 1, repeat: Infinity }} className="absolute bottom-[15px] w-5 h-1.5 bg-primary/40 rounded pointer-events-none" />;
       case 8: return <div className="absolute bottom-[20px] flex gap-1 pointer-events-none"><motion.div animate={{ height: [6, 12, 6] }} transition={{ duration: 1, repeat: Infinity }} className="w-1.5 bg-primary rounded-sm"/><motion.div animate={{ height: [8, 16, 8] }} transition={{ duration: 1, repeat: Infinity, delay: 0.2 }} className="w-1.5 bg-primary rounded-sm"/><motion.div animate={{ height: [4, 14, 4] }} transition={{ duration: 1, repeat: Infinity, delay: 0.4 }} className="w-1.5 bg-primary rounded-sm"/></div>;
       case 9: return <motion.div animate={{ rotate: [0, -10, 10, -10, 10, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 flex items-center justify-center pointer-events-none"><Bell className="w-10 h-10 text-red-500/20 blur-[2px]" /></motion.div>;
       case 10: return <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.4 + 0.5, type: "spring" }} className="absolute -top-1 -right-1 bg-green-500 rounded-full p-1 shadow-md pointer-events-none"><Check className="w-4 h-4 text-white stroke-[3]" /></motion.div>;
       default: return null;
     }
  };
